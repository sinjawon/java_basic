...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("춘식과 영희는 같은 언얼르 쓰는가 :"
                + (kim.getLang() == park.getLang()));
        System.out.println("kim의 주력 언어: " + kim.getLang().getName());
        System.out.println("park의 언어는 컴파일 언어인가? :"
                + park.getLang().isCompileLang());

        System.out.println("====================");
        //valuse(): 열거형 타입에 존재하는 모든 상수를 배열에 담아 리턴
        Language[] values = Language.values();
        for (Language value : values) {
            System.out.println(value);
        }
        //ordinal (): 열거형 타입에 열거되어 있는 순서를 정수로 리턴
